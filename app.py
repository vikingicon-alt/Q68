...
st.markdown("""
    <style>
        ...
        [data-testid="stSidebar"] { background-color: #020617; border-right: 1px solid #1e293b; min-width: 320px; }
        ...
        .neon-light { width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 14px; border: 2px solid rgba(255,255,255,0.1); opacity: 0.2; }
...
L_DB = {
    'vi': {'title': 'HỆ THỐNG Q68 TOÀN CẦU', 'pwd': 'MÃ TRUY CẬP HỆ THỐNG', 'btn_act': 'KÍCH HOẠT NGAY', 'asset': 'TÀI SẢN', 'tf': 'KHUNG GIỜ', 'style': 'BIỂU ĐỒ', 'buy': 'MUA', 'sell': 'BÁN', 'hold': 'CHỜ', 'price': 'GIÁ HIỆN TẠI', 'vol': 'KHỐI LƯỢNG'},
    'en': {'title': 'Q68 GLOBAL SYSTEM', 'pwd': 'ACCESS KEY', 'btn_act': 'ACTIVATE', 'asset': 'ASSET', 'tf': 'TIMEFRAME', 'style': 'STYLE', 'buy': 'BUY', 'sell': 'SELL', 'hold': 'HOLD', 'price': 'PRICE', 'vol': 'VOLUME'}
}
...
if not st.session_state['auth']:
    ...
    with mid:
        ...
        st.session_state['lang'] = 'vi' if st.radio(" ", ["Tiếng Việt", "English"], horizontal=True, label_visibility="collapsed") == "Tiếng Việt" else 'en'
        L = L_DB[st.session_state['lang']]
        st.markdown(f"<h1 style='text-align: center; color: gold;'>{L['title']}</h1>", unsafe_allow_html=True)
        if st.button(L['btn_act']) if st.text_input(L['pwd'], type="password") == "A1PRO" else None:
            st.session_state['auth'] = True
            st.rerun()
    st.stop()

L = L_DB[st.session_state['lang']]
# --- 3. DATA ENGINE (FULL VOLUME & RSI) ---
...
# --- 4. SIDEBAR ---
with st.sidebar:
    st.markdown("<h3 style='color: gold; text-align: center;'>Q68 SETTINGS</h3>", unsafe_allow_html=True)
...
# --- 5. HIỂN THỊ ---
df = get_full_data(target, tf_val)
if df is not None:
    ...
    if style == "Candles": fig.add_trace(go.Candlestick(x=df.index, open=df['O'], high=df['H'], low=df['L'], close=df['C'], name="Price"), row=1, col=1)
    elif style == "Line": fig.add_trace(go.Scatter(y=df['C'], line=dict(color='#3b82f6', width=2), name="Price"), row=1, col=1)
    else: fig.add_trace(go.Scatter(y=df['C'], fill='tozeroy', line=dict(color='gold'), name="Price"), row=1, col=1)
    
    fig.add_trace(go.Bar(x=df.index, y=df['V'], name="Volume", marker_color='rgba(100,100,100,0.5)'), row=2, col=1)
