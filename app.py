...
with st.sidebar:
    ...
    time_frame = st.selectbox("2. Chọn khung thời gian:", ["30 Phút", "1 Giờ", "4 Giờ", "1 Ngày", "1 Tuần"])
...
if not data.empty:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['MA20'], line=dict(color='yellow', width=1), name='MA20'), row=1, col=1)fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], line=dict(color='#e91e63', width=2), name='RSI'), row=2, col=1)
